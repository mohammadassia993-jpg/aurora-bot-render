...
async function sendTelegram({ chat_id, text }) {
  ...
  const res = await withTimeout(fetch('https://api.telegram.org/bot' + TELEGRAM_BOT_TOKEN + '/sendMessage', {
...
export async function executeTool(toolName, params = {}) {
  ...
  if (!fn) {
    ...
    logToolFailure(toolName, params, new Error(err));
    return { ok: false, tool: toolName, error: err };
  }
  try {
